...
- [x] Stripe 本番キー・Webhook設定（2025-10-23 ライブキー反映＆実カード決済テスト済み）
- [x] `ALLOWED_CHECKOUT_EMAILS` を本番運用向けに設定（テスト用ホワイトリストを解除済み、制限なしで購入可能）
- [x] カスタムドメイン + HTTPS 有効化（www.momentia.photo を本番ドメインとして稼働、TLS 証明書確認済み）
- [x] ペネトレーションテスト完了
- [x] ログ／監視／アラート 最低限稼働（Azure Monitor/Log Analytics/Stripe アラートを設定済み）
- [x] 問い合わせフォーム／決済の失敗ケース確認（2025-10-23 Stripe カード拒否/CSRF/エラー時レスポンスを検証済み、問い合わせフォームは 10/15 テスト送信で確認済み）
- [x] メール送信の到達性確認（Gmail / Outlook で検証済み。携帯キャリア向けは不要と判断）
- [ ] 本番リリース直前にテスト用 Order / AccessToken / Invoice を削除（初回リリース後にテスト済み注文が残る方針）
